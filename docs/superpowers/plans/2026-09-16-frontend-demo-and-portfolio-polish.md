...
- [x] **Step 2: Push and let Vercel deploy**
...
- **Unplanned fix, discovered starting Task 11: CI's `frontend` job was already failing before this session touched anything** (confirmed red on `da0b7b78` and `70f088e1`, both pre-dating Task 1). Root cause via GitHub's check-run annotations: `Cannot find name 'LayoutProps'` in `src/app/layout.tsx` and `src/app/(app)/layout.tsx`. `LayoutProps`/`PageProps` are ambient types Next.js writes into `.next/types` during `next dev`, `next build`, or `next typegen` (confirmed in `node_modules/next/dist/docs/.../02-typescript.md`) — CI's `frontend` job ran `npx tsc --noEmit` *before* `npm run build`, so on a fresh checkout with no `.next/` directory the type is simply undefined. Local runs never hit this because `.next/` already existed on disk from earlier manual `npm run dev`/`build` sessions predating this conversation. Fixed by adding `npx next typegen` as its own step immediately before `npx tsc --noEmit` in `.github/workflows/ci.yml`'s `frontend` job — Next's own docs name this exact command for this exact purpose. Verified locally by deleting `.next/` entirely and replaying `next typegen → tsc --noEmit → npm run build → npm test` in that order (all green), then confirmed both CI jobs green on GitHub for commit `cc585c6`. This was not one of the plan's 11 tasks; it blocked Task 11 Step 2 ("confirm both CI jobs pass"), so it was fixed at the root rather than reported and left red.
